...
/* ────────── 전역 변수 ────────── */
static uint8_t dtc_code[3]   = {0};
static uint8_t pmic_rx_buf[8];
...
static void CAN_SendDTC(const uint8_t *dtc);
...
static HAL_StatusTypeDef PMIC_ReadReg_DMA(PMIC_Reg_t reg, PMIC_RegFrame_t *dst)
{
  if (HAL_I2C_Mem_Read_DMA(&hi2c1, PMIC_I2C_ADDR, reg,
                           I2C_MEMADD_SIZE_8BIT, pmic_rx_buf, sizeof pmic_rx_buf) != HAL_OK)
      return HAL_ERROR;
  while (HAL_I2C_GetState(&hi2c1) != HAL_I2C_STATE_READY);
  memcpy(dst->raw, pmic_rx_buf, sizeof pmic_rx_buf);
  return HAL_OK;
}
...
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *hcan)
{
  ...
  if (rx.StdId == CAN_ID_DTC_REQ && data[0] == 0x14) {   /* DTC Clear */
      memset(dtc_code, 0, sizeof dtc_code);
